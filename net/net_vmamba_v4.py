...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #================clip=================#
        self.linear_hsi = nn.Linear(512, 1024)
        self.linear_lidar = nn.Linear(512, 1024)
        self.conv_hsi = nn.Conv2d(1, in_chans_hsi, 1, 1, 0)
        self.conv_lidar = nn.Conv2d(1, in_chans_lidar, 1, 1, 0)
        #================clip=================#
        ...
        #================clip=================#
        self.patch_embed_hsi_clip = _make_patch_embed(in_chans_hsi, dims, patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        self.patch_embed_lidar_clip = _make_patch_embed(in_chans_lidar, dims, patch_size, patch_norm, norm_layer, channel_first=self.channel_first)
        #================clip=================#
        ...
        #     norm=norm_layer(self.num_features), # B,H,W,C
        #     permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
        #     conv=nn.ConvTranspose2d(self.num_features, self.num_features, 4, 2, 1),  #4, 4, 0 for ps4 | 4, 2, 1 for ps2
        #     head=nn.Conv2d(self.num_features, num_classes, 1, 1) if num_classes > 0 else nn.Identity(),
        ...
        #     permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
        #     conv=nn.ConvTranspose2d(self.num_features, self.num_features, 4, 2, 1),
        #     head=nn.Conv2d(self.num_features, num_classes, 1, 1) if num_classes > 0 else nn.Identity(),
        # ))
        # self.classifier_add = nn.Sequential(OrderedDict(
        #     norm=norm_layer(self.num_features), # B,H,W,C
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, x_clip: torch.Tensor, y_clip: torch.Tensor):
        x_clip = self.conv_hsi(self.linear_hsi(x_clip).reshape(x_clip.shape[0], -1, 32, 32))
        y_clip = self.conv_lidar(self.linear_lidar(y_clip).reshape(y_clip.shape[0], -1, 32, 32))
        x = self.patch_embed_hsi(x)
        y = self.patch_embed_lidar(y)
        x_clip = self.patch_embed_hsi_clip(x_clip)
        y_clip = self.patch_embed_lidar_clip(y_clip)
        z = x_clip + y_clip
    ...
    def flops(self, shape=(3, 640, 480)):
